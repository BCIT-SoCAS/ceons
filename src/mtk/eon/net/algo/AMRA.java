...
import mtk.eon.net.CPUException;
import mtk.eon.net.MemoryException;
...
import mtk.eon.net.StorageException;
	...
	public DemandAllocationResult allocateDemand(Demand demand, Network network) {
		...
		try {
			if (demand.allocateBackup()) {
				...
				if (candidatePaths.isEmpty())
					return new DemandAllocationResult(DemandAllocationResult.Type.NO_REGENERATORS,
			...
			return new DemandAllocationResult(DemandAllocationResult.Type.NO_CPU, demand.getWorkingPath());
		} catch (MemoryException memory) {
			return new DemandAllocationResult(DemandAllocationResult.Type.NO_MEMORY, demand.getWorkingPath());
		} catch (StorageException storage) {
			return new DemandAllocationResult(DemandAllocationResult.Type.NO_STORAGE, demand.getWorkingPath());
		}
