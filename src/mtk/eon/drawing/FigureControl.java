...
public class FigureControl {
    ...
    public void add(Figure temp) {
            ...
            if (temp instanceof Node) {
                ...
                if (temp instanceof Link) {
                    Vector2F p = list.get(findClosestNode(temp.getStartPoint())).getStartPoint();
                    p = fixLinkPoint(p);
    ...
    private double calculateDistance(int idx,Vector2F p)
    {
        ...
        return Math.sqrt(Math.pow(p.getX()-temp.getStartPoint().getX(),2)+Math.pow(p.getY() - temp.getStartPoint().getY(), 2));
    }
    public void redraw() {
        clearCanvas();
        for (Figure fig:list)
    ...
    {
        ((Link)list.get(list.size()-1)).setEndPoint(p);
        redraw();
    }
    public void changeLinkEndPointAfterDrag(Vector2F p)
    {
        ...
        Link link=((Link)list.get(list.size()-1));
    ...
    {
        Vector2F vec2F=new Vector2F((float)canvas.getWidth()/2,(float)canvas.getHeight()/2);
    ...
    public Matrix2F allElementsToMatrix(Vector2F rotatePoint )
    {
        ...
        for (Figure fig:list)
        {
            ...
            if(fig instanceof Link)
            {
                table[actualFigure][0]=table[actualFigure][0]-Node.imageSize/2;
                table[actualFigure][1]=table[actualFigure][1]-Node.imageSize/2;
                actualFigure++;
                table[actualFigure][0]=((Link) fig).getEndPoint().getX()-dx-Node.imageSize/2;
                table[actualFigure][1]=((Link) fig).getEndPoint().getY()-dy-Node.imageSize/2;
            }
            actualFigure++;
        }
    ...
    public void matrixToList(Matrix2F mat, Vector2F rotationPoint)
    {
        ...
        for(Figure fig:list)
        {
            ...
            Vector2F fixedVector=new Vector2F((int)(temp.getX()+dx),(int)(temp.getY()+dy));
            fig.setStartPoint(fixedVector);
            if(fig instanceof Link) {
                fixedVector=list.get(findClosestNode(fixedVector)).getStartPoint();
                fig.setStartPoint(new Vector2F(fixedVector.getX()+Node.imageSize/2,fixedVector.getY()+Node.imageSize/2));
                actualFigure++;
                temp=mat.getRow(actualFigure);
                fixedVector=new Vector2F((int)(temp.getX()+dx),(int)(temp.getY()+dy));
                fixedVector=list.get(findClosestNode(fixedVector)).getStartPoint();
                ((Link) fig).setEndPoint(new Vector2F((int) (fixedVector.getX() + Node.imageSize / 2), (int) (fixedVector.getY() + Node.imageSize / 2)));
            }
            actualFigure++;
        }
        redraw();
    ...
    private boolean areCrossing(Link fig, Link link) {
        Vector2F p1=fig.getStartPoint();
        Vector2F p2=fig.getEndPoint();
        Vector2F p3=link.getStartPoint();
        Vector2F p4=link.getEndPoint();
    ...
    public void deleteNode(Vector2F clickedPoint) {
        ...
        {
        Vector2F fixedPoint = fixLinkPoint(list.get(temp).getStartPoint());
            for (int i = 0; i < list.size(); i++) {
                Figure fig = list.get(i);
                if (fig instanceof Link) {
                    if (fig.getStartPoint().equals(fixedPoint) || ((Link) fig).getEndPoint().equals(fixedPoint)) {
                        list.remove(i);
                        i--;
                    }
                }
    ...
    public void deleteElementsFromRectangle(Vector2F startPoint, Vector2F endPoint) {
        Vector2F temp1=new Vector2F(Math.min(startPoint.getX(),endPoint.getX()),Math.min(startPoint.getY(),endPoint.getY()));
        Vector2F temp2=new Vector2F(Math.max(startPoint.getX(), endPoint.getX()),Math.max(startPoint.getY(),endPoint.getY()));
        startPoint=temp1;
        endPoint=temp2;
        Link rectSide1=new Link(new Vector2F(startPoint.getX(),startPoint.getY()),new Vector2F(endPoint.getX(),startPoint.getY()),"rectSide1");
        Link rectSide2=new Link(new Vector2F(startPoint.getX(),startPoint.getY()),new Vector2F(startPoint.getX(),endPoint.getY()),"rectSide2");
        Link rectSide3=new Link(new Vector2F(endPoint.getX(),startPoint.getY()), new Vector2F(endPoint.getX(),endPoint.getY()),"rectSide3");
        Link rectSide4=new Link(new Vector2F(startPoint.getX(),endPoint.getY()), new Vector2F(endPoint.getX(),endPoint.getY()),"rectSide4");
        for (int i=0;i<list.size();i++)
        {
            Figure fig=list.get(i);
            if(fig.getStartPoint().getX()>startPoint.getX() &&
               ...
               fig.getStartPoint().getY()<endPoint.getY()) {
                ...
                if((((Link) fig).getEndPoint().getX()>startPoint.getX() &&
                   ...
                   ((Link) fig).getEndPoint().getY()>startPoint.getY() &&
                   ((Link) fig).getEndPoint().getY()<endPoint.getY()) || areCrossing((Link)fig,rectSide1) || areCrossing((Link)fig,rectSide2) ||areCrossing((Link)fig,rectSide3) || areCrossing((Link)fig,rectSide4))
                {
                list.remove(i);
                i--;
            }
