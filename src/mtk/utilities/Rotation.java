...
public class Rotation {
	...
	public FigureControl rotate(Vector2F startPoint,Vector2F endPoint)
	{
		...
		Matrix2F rotationTable=new Matrix2F(new float[][]{{(float) Math.cos(rotationArc), -(float) Math.sin(rotationArc)}, {(float) Math.sin(rotationArc), (float) Math.cos(rotationArc)}});
