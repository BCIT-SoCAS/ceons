...
public class YamlConfiguration {
	...
	private static Object deserializeAll(Object object) {
		if (object instanceof Map) {
			...
			for (int i = 0; i < list.size(); i++)
				list.set(i, deserializeAll(list.get(i)));
	...
	private static Object serializeAll(Object object) {
		if (object instanceof Map) {
			...
			List list = (List) object;
			for (int i = 0; i < list.size(); i++)
				list.set(i, serializeAll(list.get(i)));
	...
	public <T> T get(String path) {
		...
		if (splittedPath.length == 0)
			splittedPath = new String[] {path};
		Object result = config;
		for (String aSplittedPath : splittedPath)
			if (result instanceof Map)
				result = ((Map) result).get(aSplittedPath);
	...
	public <T> T put(String path, Object value) {
		...
		if (container instanceof Map)
			...
			if (index == ((List) container).size()) {
				...
				return (T) ((List) container).set(index, value);
		}
		else
			throw new YAMLException("Tried to store a value in an non-existent container!");
	}
	...
	public <T> T remove(String path) {
		...
		if (container instanceof Map)
			...
			throw new YAMLException("Tried to remove a value from an unexisting container!");
	}
